...
public class Updater {
    ...
    public static string Dir(int year, int day) => SolverExtensions.WorkingDir(year, day);
    ...
    void UpdateTest(Problem problem)
    {
        var dir = Path.Combine("../tests/", Dir(problem.Year, problem.Day));
        if (!Directory.Exists(dir)) {
            Directory.CreateDirectory(dir);
        }
        var file = Path.Combine(dir, "Tests.cs");
        if (!File.Exists(file)) {
            WriteFile(file, new TestTemplateGenerator().Generate(problem));
        }
    }
